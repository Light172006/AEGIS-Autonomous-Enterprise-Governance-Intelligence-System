...
class BackendConfig:
    ...
    def from_env(cls) -> "BackendConfig":
        ...
        # Make SQLite URL absolute so the DB file is always in the project root
        db_url = os.getenv("AEGIS_DATABASE_URL", "sqlite:///aegis.db")
        if db_url.startswith("sqlite:///") and not Path(db_url.replace("sqlite:///", "")).is_absolute():
            db_url = f"sqlite:///{PROJECT_ROOT / db_url.replace('sqlite:///', '')}"
